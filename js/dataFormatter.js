function formatData(data){
  ...
  for(var i = 0, l = eachLine.length; i < l; i++) {
      ...
      if(!leadingWhitespace){
        if(eachLine[i].indexOf("[ON_CAMPUS]") >= 0){
          formatted += "<h5>On Campus</h5>";
        }else if(eachLine[i].indexOf("[OFF_CAMPUS]") >= 0){
          formatted += "<h5>Off Campus</h5>";
        }else{
  ...
  formatted = formatted.replace("[ON_CAMPUS]", "<h5>On Campus</h5>");
  formatted = formatted.replace("[OFF_CAMPUS]", "<h5>Off Campus</h5>");
